...
void MT_UartProcessZToolData ( uint8 port, uint8 event )
{
  ...
  //²éÑ¯»º³åÇø¶ÁÐÅÏ¢£¬Ò²³ÉÁËÕâÀïÐÅÏ¢ÊÇ·ñ½ÓÊÜÍê³ÉµÄ±êÖ¾
  while (Hal_UART_RxBufLen(port))    
  {
    HalUARTRead (port, &ch, 1);
    //Ò»¸öÒ»¸öµÄ¶Á£¬¶ÁÍêÒ»¸ö»º³åÇø¾ÍÇåÒ»¸ö
    switch (state)     //Õâ±ßÊÇ×´Ì¬»ú
    {
      case SOP_STATE:
        if (ch == MT_UART_SOF)   //MT_UART_SOFµÄÖµÄ¬ÈÏÊÇ0xfe,ËùÒÔÊý¾Ý±ØÐëFE¸ñÊ½¿ªÊ¼·¢ËÍ²ÅÄÜ½øÈëÏÂÒ»×´Ì¬£¬²»È»ÓÀÔ¶ÔÚÕâÀï×ªÈ¦
          state = LEN_STATE;
        break;
      ...
      case LEN_STATE:
        ...
        /* Allocate memory for the data    ·ÖÅäÄÚ´æ¿Õ¼ä */
        ...
        if (pMsg)
        {
          ...
          //×¢²áÊÂ¼þºÅCMD_SERIAL_MSG; ºÜÓÐÓÃµÄ
      ...
      case FCS_STATE:
        ...
        if ((MT_UartCalcFCS ((uint8*)&pMsg->msg[0], MT_RPC_FRAME_HDR_SZ + LEN_Token) == FSC_Token)) //Êý¾ÝÐ£Ñé
        {
          osal_msg_send( App_TaskID, (byte *)pMsg );  //°ÑÊý¾Ý°ü·¢ËÍµ½OSAL²ã¡£·Ç³£ÖØÒª
        }
        else
        {
          /* deallocate the msg */
          osal_msg_deallocate ( (uint8 *)pMsg );   //Çå³ýÉêÇëµÄÄÚ´æ¿Õ¼ä
        }

        /* Reset the state, send or discard the buffers at this point */
        state = SOP_STATE;  //×´Ì¬»úÒ»ÖÜÆÚÍê³É
